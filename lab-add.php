        ...
        <div class="content-sec">
            ...
            <div class="content-area">
                ...
                <div class="top-bar">
                    <div class="top-bar-area">
                        <div class="top-bar-title-area">
                            <h1 class="top-bar-title font-size-medium font-weight-medium color-black">Add Lab</h1>
                            <p class="top-bar-title-desc font-size-regular font-weight-regular color-black"><a href="lab.php">Lab</a> >> Setup Lab</p>
                        </div>
                    </div>
                </div>
                <!-- // Form >>>> -->
                ...
                if (isset($_POST["submit"])) {
                    ...
                    # Value check
                    if (empty($department) || empty($subject) || empty($startTime) || empty($endTime) || empty($year)) {
                        ?>
                        <div class="fail-alert-box alert-box">
                            ...
                            <div class="alert-box-details">
                                ...
                                <p class="alert-box-desc font-size-regular font-family-regular color-black">All * fields are mandatory.</p>
                            </div>
                        ...
                        $check_slot_sql = $sql->prepare("SELECT * FROM `lab` WHERE `year_id` = ? AND `department_id` = ? AND `subject_id` = ? AND NOT (? >= `end_time` OR ? <= `start_time`)");
                        $check_slot_sql->bind_param('sssss', $year, $department, $subject, $startTime, $endTime);
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            # Inserting
                            $insert_lab_sql = $sql->prepare("INSERT INTO `lab`(`year_id`, `department_id`, `subject_id`, `start_time`, `end_time`) VALUES (?,?,?,?,?)");
                            $insert_lab_sql->bind_param('sssss', $year, $department, $subject, $startTime, $endTime);
                            $insert_lab_sql->execute();
                            $insert_info = $insert_lab_sql->get_result();
                            if ($insert_lab_sql->affected_rows > 0) {
                                $year = $department = $subject = $startTime = $endTime = '';
                                ?>
                                <div class="succ-alert-box alert-box">
                                    ...
                                    <div class="alert-box-details">
                                        ...
                                        <p class="alert-box-desc font-size-regular font-family-regular color-black">Lab setup successfully.</p>
                                    </div>
                                    <img src="images/cross.png" alt="Cross" class="alert-box-close-img">
                                    <script>
                                        // Default Hide alert
                ...
                <form action="" method="POST" enctype="multipart/form-data" class="form-area">                  
                        ...
                        <div class="single-input">
                            <button type="submit" name="submit" class="primary-btn form-submit-btn">Add Lab</button>
                        </div>
                    </form>
                </div>
            </div>
