...
if (isset($_POST['id'])) {
    $id = $_POST['id'];
    $entries_del_sql = $sql->prepare("DELETE FROM `entries` WHERE e_id = ?");
    $entries_del_sql->bind_param('s', $id);
    $entries_del_sql->execute();
    if ($entries_del_sql->affected_rows > 0) {
?>
        <div class="succ-alert-box alert-box">
            ...
            <div class="alert-box-details">
                ...
                <p class="alert-box-desc font-size-regular font-family-regular color-black">Entry deleted successfully.</p>
            </div>  
